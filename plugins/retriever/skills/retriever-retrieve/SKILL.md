...
- If `requires_reinstall_cleanup` is true, do not use retained state. In an interactive chat, direct the user to select **Start a fresh private job search**; in a scheduled task, skip without opening Chrome or writing local state.
...
- If it is false in a scheduled task, state that the scan was skipped because Retriever needs interactive onboarding. Do not invoke Chrome, `run start`, `run finish`, job writes, or report writes. Direct the user to start a Codex chat and select `Start a fresh private job search`.
...
9. Finish the retrieval run with completed or error status. A rescan updates a matching job's latest sighting and observation history, but it must never change a user-approved archive decision.
10. For every successful retrieval—interactive or scheduled—always start or reuse the local interactive dashboard with `python3 <plugin-root>/scripts/retriever.py dashboard start --ranked`, then show its returned URL prominently when reporting the run. The managed dashboard is a detached loopback-only service and remains available after a scheduled task finishes; do not open an external careers page in place of this local dashboard. If it cannot start, state that clearly and still provide a safe read-only report. In every case, report the run count and visible-job count. If there are more than six visible jobs, show at most six ranked matches by default, then explicitly offer the complete database and CSV export. Do not imply the short list is the whole result set.
