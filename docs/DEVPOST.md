...
- Ask Retriever to reset for a clean test, review the explicit preview of its own schedules and local artifacts, then confirm that the next launch begins fresh onboarding without manually deleting `~/.retriever`.
- Verify an unconfigured scheduled run skips cleanly without opening Chrome or recording an error run; then verify daily, weekly, and monthly **machine-local** cadence plans update one Retriever-owned task, named timezones request local-time confirmation rather than silent conversion, and the configured schedule resolves the currently installed Retriever runtime at execution time rather than retaining a versioned cache path. After a successful configured run, verify it presents the managed local dashboard URL rather than an employer careers page.
